...
Game::Game()
{
    m_player_tex = load_texture("./assets/G.bmp", g_renderer);
...
{
    if (m_player_tex) {
        SDL_RenderCopyEx(g_renderer, m_player_tex, nullptr, &player_rect, player_rotation, nullptr,
                         SDL_FLIP_NONE);
    }
...
void Game::update(Uint32 delta)
{
    ...
    if (INPUT.KEY_W) {
        player_pos.y -= PLAYER_MOVEMENT_SPEED * delta_f;
    }
    else if (INPUT.KEY_S) {
        player_pos.y += PLAYER_MOVEMENT_SPEED * delta_f;
    }
    ...
    if (INPUT.KEY_A) {
        player_pos.x -= PLAYER_MOVEMENT_SPEED * delta_f;
    }
    else if (INPUT.KEY_D) {
        player_pos.x += PLAYER_MOVEMENT_SPEED * delta_f;
    }

    player_rect.x = static_cast<Sint32>(player_pos.x - 20.0f);
    player_rect.y = static_cast<Sint32>(player_pos.y - 20.0f);
